  ...
  class MathJaxWasmEngine {
      ...
      try {
        ...
        const results = await WebAssembly.instantiate(bytes, {
          env: {
            ...
            abort: () => console.error('[MathJax-WASM] WebAssembly abort called'),
          }
        });
        ...
        console.warn('[MathJax-WASM] Wasm fallback to native MathJax 3.2.2:', err);
        return false;
      }
    }
    ...
    texToSvg(texString, isDisplayMode = true) {
      if (window.MathJax && window.MathJax.tex2svg) {
        const svgNode = window.MathJax.tex2svg(texString, { display: isDisplayMode });
        return svgNode.outerHTML;
      }
      const modeAttr = isDisplayMode ? 'block' : 'inline';
      return `<svg class="mathjax-wasm-svg ${modeAttr}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 20"><text x="10" y="15" fill="currentColor" font-family="monospace">${texString}</text></svg>`;
    }
